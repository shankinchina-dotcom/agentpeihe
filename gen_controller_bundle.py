...
检测本机可用执行体（CLI、API key、真 Claude vs CC Switch 劫持、注册表 cooldown），
...
规则:
  - 大脑默认优先级: codex → claude-sdk（仅当真 anthropic）→ pi(deepseek)
  - 子 agent 按真实 vendor 命名；Reviewer 必须异 vendor
  - 真 Claude: claude-native + permission_mode=auto（仅 vendor=anthropic）
  - 禁止用 claude-native/claude-sdk 伪装 Kimi/DeepSeek（CC Switch 劫持不进工人池）
  - Kimi: 只走 kimi-native（手工 omnigent kimi + 自动派 exec_moonshot）；默认 --yolo
  - DeepSeek: pi + provider deepseek 官方 API
...
{pool_table}
  ...
  ## 通道隔离（硬规则）
  - Claude 只走 anthropic（claude-native / claude-sdk），禁止 CC Switch 劫持到 Kimi/DeepSeek
  - DeepSeek 只走 pi + 官方 API（providers.deepseek）
  - Kimi 只走 kimi-native（手工：网页选 Kimi / `omnigent-zh kimi`；自动：exec_moonshot）
  - 禁止把 Kimi 塞进 Claude 壳
...
def build(det: dict, brain: str | None) -> tuple[dict[str, str], list[str], str, list[str]]:
    ...
    # 真 Claude 工人：仅 vendor=anthropic 才进池；CC Switch 劫持一律不进
    if det["has_claude"] and det["claude_vendor"] == "anthropic":
        workers.append(
            ("exec_anthropic", "claude-native", "anthropic", "真 Claude Code，permission_mode=auto")
        )
        files["agents/exec_anthropic/config.yaml"] = worker_yaml(
            "exec_anthropic",
            "anthropic 执行体（真 Claude Code）。可担任 executor/reviewer。",
            "    harness: claude-native\n    permission_mode: auto",
            WORKER_PROMPT,
        )
    elif det["has_claude"] and det["claude_vendor"] and det["claude_vendor"] != "anthropic":
        pool_notes.append(
            f"  | claude-native | claude-native | {det['claude_vendor']} | — | "
            f"不可用（CC Switch 劫持，禁止进池；请清 ANTHROPIC_BASE_URL 并登录真 Claude） |"
        )
    ...
    # pi API 工人（DeepSeek 等官方 gateway）
    if det["has_pi"]:
        for provider, model, vendor in det["pi_providers"]:
            workers.append((f"exec_{vendor}", f"pi:{model}", vendor, "API key 已检测到"))
